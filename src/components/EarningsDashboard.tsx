...
const PIE_COLORS = ["#00f0ff", "#22d3ee", "#0891b2", "#a78bfa", "#67e8f9", "#7c3aed"];
  ...
  if (error || !data || data.anonymous || data.balance === null) {
    return (
      <div className="glass-card flex flex-col items-center justify-center py-16 text-center">
        <Wallet className="mb-3 h-10 w-10 text-cyan/50" />
        ...
        <div className="glass-card">
          <div className="mb-2 flex items-center justify-between text-xs">
            <span className="text-white/60">
              Progress to <strong className="text-cyan">{data.nextTier.name}</strong> tier
            </span>
      ...
      <div className="glass-card">
        <div className="mb-4 flex items-center justify-between">
          ...
          <a
            ...
            className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-white/70 transition-colors hover:border-cyan/50 hover:text-cyan"
          >
            ...
            <tbody>
              ...
              {data.payouts.slice(0, 20).map((p) => (
                <tr key={p.id} className="border-b border-white/5 font-mono text-xs">
                  ...
                  <td className="py-2 text-right text-cyan">+{p.amount}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function StatCard({
  ...
  return (
    <motion.div
      ...
      className={cn("glass-card", accent && "border-cyan/30 shadow-glow-sm")}
    >
      <div className="mb-2 flex items-center gap-2 text-xs text-white/40">
        <Icon className="h-3.5 w-3.5" />
        {label}
      </div>
      <div className={cn("text-2xl font-semibold", accent ? "text-cyan" : "text-white")}>
        {value}
      </div>
    </motion.div>
  );
