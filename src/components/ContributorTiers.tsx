        ...
        return (
          <motion.div
            ...
            className={cn(
              ...
              featured ? "glass border-hive/40 shadow-glow" : "glass",
            )}
          >
            {featured && (
              <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-hive px-3 py-1 text-xs font-semibold text-black">
                Top Tier
              </span>
            )}
            ...
            <div className="mb-1 flex items-center gap-2">
              ...
              <Zap className="h-4 w-4 text-hive" />
            </div>

            <div className="mb-1 text-sm text-white/50">{tier.blurb}</div>

            <div className="mb-4">
              <span className="text-4xl font-bold text-hive">{tier.multiplier}×</span>
            ...
            <ul className="mb-6 flex-1 space-y-2.5">
              {tier.perks.map((f) => (
                <li key={f} className="flex items-start gap-2 text-sm text-white/70">
                  <Check className="mt-0.5 h-4 w-4 shrink-0 text-hive" />
                  {f}
                </li>
              ))}
            </ul>
            ...
            {isCurrent ? (
              <div className="rounded-xl border border-hive/40 bg-hive/10 py-3 text-center text-sm font-medium text-hive">
                Your Tier
              </div>
            ) : (
              <a
                ...
                className={cn(
                  ...
                  featured
                    ? "btn-glow w-full"
                    : "border border-white/15 bg-white/5 text-white hover:border-hive/50 hover:text-hive",
                )}
              >
                Rate to unlock
              </a>
